...
- **Blocage D3** :
  ...
  - DevTools ▸ Network ▸ onglet *Request blocking* ➜ ajouter `*d3*`.
  - Recharger et vérifier :
    - message d’erreur explicite,
    - bouton « Réessayer » actif,
    - affichage de la grille des départements.
  - Cliquer « Réessayer » : observer le loader, puis le fallback image si l’échec persiste.
  - Retirer le blocage, recharger et confirmer que la carte SVG s’affiche normalement.

- **Tests navigateurs** :
  - Chrome (desktop + mobile emulation), Firefox, Edge, Safari (si possible).
  - Contrôler : rendu de la carte, hover/clic départements, mises à jour du panneau d’info.
...
- **Parcours principaux** :
  ...
  - Tester entrée valide ➜ confirmation + reset + disparition des erreurs.

- **Validation en ligne** :
  - Laisser des champs requis vides ➜ message « … est obligatoire ».
  - Email invalide ➜ message format.
  - Téléphone invalide ➜ message format FR.
  - Villes < 2 caractères ➜ message adéquat.

- **Notifications** :
  - Succès ➜ notif verte (icône check).
  - Erreur validation ➜ notif rouge « Veuillez corriger… ».
  - Fermeture manuelle (bouton ×) et auto (5s pour info/success).

- **Gestion réseau** :
  - DevTools ▸ Network ▸ profil *Offline* ➜ message « Vous semblez hors connexion… », aucun appel réseau.
  - Profil *Slow 3G* ➜ timeout ≈ 15s ➜ message délai + suggestion.
  - Simuler HTTP 500/502 (via mock/proxy) ➜ message serveur temporaire + coordonnée support.
  - Simuler HTTP 429 ➜ message limitation.
...
- **Duplication backend** :
  ...
  - Confirmer que le système de notification/email n’envoie qu’un exemplaire.
