...
// Mapping d̩partements -> villes disponibles
// ============================================

// Mapping des d̩partements vers les villes disponibles (SANS CORSE)
...
// Noms des d̩partements (SANS CORSE)
...
async function loadFranceMapSVG() {
    try {
        ...
        console.log('SVG non trouv̩, utilisation de la carte g̩n̩r̩e');
    }
    return null;
}

// Cr̩er la carte SVG de France avec d̩partements
function createFranceMapSVG() {
    const container = document.getElementById('france-map');
    if (!container) return;
    
    // SVG simplifi̩ de la France (version basique mais fonctionnelle)
    // Pour une vraie carte d̩taill̩e, il faudrait un fichier SVG complet
    const svg = `
        <svg viewBox="0 0 800 1000" xmlns="http://www.w3.org/2000/svg" class="france-svg-map">
            ...
            <!-- Note: Ceci est un SVG simplifi̩. Pour une vraie carte, utilisez un SVG complet de la France -->
            <text x="400" y="500" text-anchor="middle" class="dept-text" font-size="24">Carte de France</text>
            <text x="400" y="530" text-anchor="middle" class="dept-text" font-size="16" fill="#6b7280">Chargement de la carte d̩taill̩e...</text>
        </svg>
    `;
    ...
    loadFranceMapSVG().then(svgContent => {
        if (svgContent) {
            ...
            // Si pas de SVG, cr̩er une grille interactive
            createInteractiveGrid();
        }
    });
}

// Cr̩er une grille interactive si pas de SVG
...
function initMapInteractions() {
    ...
    svgPaths.forEach(element => {
        ...
        // Style pour d̩partements avec villes
...
// Mettre en surbrillance un d̩partement
...
// G̩rer le clic sur un d̩partement - Redirection directe
